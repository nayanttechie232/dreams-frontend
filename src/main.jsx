...
function App() {
  ...
  const [cardReviewDream, setCardReviewDream] = useState(null);
  const [cardReviews, setCardReviews] = useState([]);
  const [cardReviewsLoading, setCardReviewsLoading] = useState(false);
  const [cardReviewsOpen, setCardReviewsOpen] = useState(false);
  ...
  const openReviews = async () => {
    if (dream._id === 'welcome') return;
    setReviewsLoading(true);
    setScreen('reviews');
    window.scrollTo({ top: 0, behavior: 'smooth' });
    try { setReviews(await call(`/dreams/${dream._id}/reviews`)); } catch (err) { alert(err.message); } finally { setReviewsLoading(false); }
  };
  const submitReview = async event => {
    event.preventDefault();
    setReviewError('');
    try {
      const created = await call('/reviews', { method: 'POST', body: JSON.stringify({ dreamId: dream._id, text: reviewText }) });
      setReviews(current => [created, ...current]);
      setReviewText('');
      setReviewOpen(false);
    } catch (err) { setReviewError(err.message); }
  };
  const openCardReviews = async item => {
    if (!user) { setAuthMode('login'); setAuthOpen(true); return; }
    setCardReviewDream(item);
    setCardReviews([]);
    setCardReviewsLoading(true);
    setCardReviewsOpen(true);
    try { setCardReviews(await call(`/dreams/${item._id}/reviews`)); } catch (err) { alert(err.message); setCardReviewsOpen(false); } finally { setCardReviewsLoading(false); }
  };
  ...
  let content;
  if (screen === 'home') content = <Home dreams={dreams} loading={dreamsLoading} onOpen={openDream} onCreate={createDream} onReviews={openCardReviews} />;
  ...
  else if (screen === 'reviews') content = <Reviews dream={dream} reviews={reviews} loading={reviewsLoading} onBack={() => { setScreen('dream'); window.scrollTo({ top: 0, behavior: 'smooth' }); }} onWrite={() => { setReviewError(''); setReviewOpen(true); }} />;
  ...
  return <><div className="star-field" />{pageLoading && <div className="page-loader" aria-label="Turning the page"><span className="loader-moon" /><p>turning the page</p></div>}<header><button className="brand" onClick={goHome} aria-label="PaperMoon home"><span className="moon" /><span>paper</span><b>moon</b></button><nav>{admin && <button className="admin" onClick={openAdmin}>Admin</button>}<button className="new" onClick={createDream}>Write a dream</button>{user ? <button className="account" onClick={() => { localStorage.clear(); setUser(null); setToken(null); }}>@{user.username} · sign out</button> : <button className="account" onClick={() => { setError(''); setNotice(''); setAuthOpen(true); }}>Sign in</button>}</nav></header>{content}{screen === 'dream' && pageIndex === dream.pages.length - 1 && <button className="end-review-button" onClick={openReviews}>Reviews <span>→</span></button>}{screen === 'dream' && dream._id !== 'welcome' && <MoonPet onClick={() => { setReviewError(''); setReviewOpen(true); }} />}{authOpen && <Auth form={form} setForm={setForm} mode={authMode} setMode={setAuthMode} error={error} notice={notice} clearMessages={() => { setError(''); setNotice(''); }} close={() => setAuthOpen(false)} submit={authenticate} />}{reviewOpen && <ReviewPrompt text={reviewText} setText={setReviewText} error={reviewError} close={() => setReviewOpen(false)} submit={submitReview} />}{cardReviewsOpen && <CardReviewPopup dream={cardReviewDream} reviews={cardReviews} loading={cardReviewsLoading} close={() => setCardReviewsOpen(false)} />}</>;
}

const HERO_LINES = ['A quieter place for your dreams.', 'And ur stories too'];

function Home({ dreams, loading, onOpen, onCreate, onReviews }) {
  const columns = Array.from({ length: 4 }, () => []);
  dreams.forEach((item, index) => columns[index % 4].push({ item, index, row: Math.floor(index / 4) }));
  const card = ({ item, index, row }, column) => <div className="dream-card-shell" key={item._id}><button className={`story-card ${(column + row) % 2 ? 'tall' : 'short'}`} style={{ '--delay': `${index * 80}ms` }} onClick={() => onOpen(item)}><img src={item.coverImage || defaultCover} alt="" /><span className="card-shade" /><div><small>BY {item.author?.username}</small><strong>{item.title}</strong><em>{item.subtitle || 'A dream in the making'}</em></div></button><button className="card-review-star" type="button" onClick={() => onReviews(item)} aria-label={`View reviews for ${item.title}`} title="Dream reviews">★</button></div>;
  return <main className="home"><section className="hero"><div className="home-moon" aria-hidden="true" /><span className="hero-star star-one" aria-hidden="true">✦</span><span className="hero-star star-two" aria-hidden="true">✧</span><span className="hero-star star-three" aria-hidden="true">✦</span><p className="eyebrow">DREAMS, PAGE BY PAGE</p><h1><Typewriter lines={HERO_LINES} /></h1><p>PaperMoon lets you create visual dreams with words and photographs, arranged like a small printed journal.</p><button className="hero-button" onClick={onCreate}>Start dreaming <span>→</span></button></section><section className="library"><div className="library-head"><p className="eyebrow">EXPLORE THE DREAMS</p><p>{dreams.length} dreams waiting to be opened</p></div><div className="card-grid">{loading && dreams.length === 0 ? <div className="dreams-loader"><span className="loader-moon" /><p>Gathering dreams</p><small>Waking up the archive…</small></div> : dreams.length ? columns.map((column, index) => <div className="dream-column" key={index}>{column.map(entry => card(entry, index))}</div>) : <div className="empty-dreams"><strong>The wall is quiet.</strong><span>Be the first to begin a dream.</span></div>}</div></section></main>;
}

function Typewriter({ lines }) { const [value, setValue] = useState(''); useEffect(() => { let line = 0; let character = 0; let timer; let stopped = false; const write = () => { if (stopped) return; const current = lines[line]; if (character < current.length) { character += 1; setValue(current.slice(0, character)); timer = setTimeout(write, 118); } else timer = setTimeout(erase, 2500); }; const erase = () => { if (stopped) return; const current = lines[line]; if (character > 0) { character -= 1; setValue(current.slice(0, character)); timer = setTimeout(erase, 70); } else { line = (line + 1) % lines.length; timer = setTimeout(write, 680); } }; timer = setTimeout(write, 500); return () => { stopped = true; clearTimeout(timer); }; }, [lines]); return <span className="typewriter" aria-label={lines.join(' / ')}>{value}<i aria-hidden="true" /></span>; }
...
function CardReviewPopup({ dream, reviews, loading, close }) { return <div className="modal card-review-modal"><section className="card-review-panel" role="dialog" aria-modal="true" aria-label="Dream reviews"><button className="close" onClick={close} aria-label="Close reviews">×</button><span className="card-review-moon">★</span><p className="eyebrow">READER NOTES</p><h2>{dream?.title || 'Dream'} reviews</h2><p className="auth-copy">{loading ? 'Gathering reader notes…' : `${reviews.length} written review${reviews.length === 1 ? '' : 's'}`}</p><div className="card-review-scroll">{loading ? <div className="visit-loading"><span className="loader-moon" /></div> : reviews.length ? reviews.map(review => <article className="card-popup-review" key={review._id}><div><strong>@{review.author?.username || 'reader'}</strong><time>{new Date(review.createdAt).toLocaleString()}</time></div><p>{review.text}</p></article>) : <div className="no-visits">No reviews have been written yet.</div>}</div></section></div>; }
function Reviews({ dream, reviews, loading, onBack, onWrite }) { return <main className="admin-page reviews-page"><button className="back" onClick={onBack}>← Back to dream</button><p className="eyebrow">READER NOTES</p><h1>{dream.title}</h1><div className="reviews-heading"><p className="admin-intro">{loading ? 'Gathering reviews…' : `${reviews.length} reader review${reviews.length === 1 ? '' : 's'}`}</p><button className="visit-button" onClick={onWrite}>Leave a review <span>→</span></button></div>{loading ? <div className="visit-loading"><span className="loader-moon" /></div> : <div className="review-list">{reviews.length ? reviews.map(review => <article className="review-row" key={review._id}><div className="review-avatar">{review.author?.username?.slice(0, 1).toUpperCase() || '?'}</div><div><strong>@{review.author?.username || 'reader'}</strong><time>{new Date(review.createdAt).toLocaleString()}</time><p>{review.text}</p></div></article>) : <div className="no-visits">No reviews yet. Be the first reader to leave a note.</div>}</div>}</main>; }
function MoonPet({ onClick }) { return <button className="moon-pet" onClick={onClick} aria-label="Leave a review"><span className="pet-speech">Leave a little note</span><span className="pet-tail" /><span className="pet-body"><span className="pet-ear pet-ear-left" /><span className="pet-ear pet-ear-right" /><span className="pet-eye pet-eye-left" /><span className="pet-eye pet-eye-right" /><span className="pet-cheek pet-cheek-left" /><span className="pet-cheek pet-cheek-right" /></span></button>; }
function ReviewPrompt({ text, setText, error, close, submit }) { return <div className="modal review-modal"><form onSubmit={submit}><button type="button" className="close" onClick={close}>×</button><div className="review-prompt-moon" /><p className="eyebrow">A NOTE FOR THE DREAMER</p><h2>What stayed with you?</h2><p className="auth-copy">Your review will appear on this dream’s reader-notes page.</p><textarea value={text} maxLength="600" placeholder="Write a kind, honest review…" onChange={event => setText(event.target.value)} autoFocus />{error && <small className="error">{error}</small>}<button className="submit">Send review <span>→</span></button></form></div>; }
