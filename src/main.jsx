...
function App() {
  ...
  const own = user && (dream.author?._id === user.id || dream.author?.username === user.username);
  ...
  const loadDreams = async () => {
    ...
    try {
      ...
      // A cached list stays visible while Render wakes up or the network reconnects.
    } finally {
  ...
  const authenticate = async event => {
    ...
    try {
      ...
      setUser(data.user); setToken(data.token); localStorage.setItem('pm-user', JSON.stringify(data.user)); localStorage.setItem('pm-token', data.token); setAuthOpen(false);
    } catch (err) { setError(err.message); }
  };

  const openDream = async summary => {
    window.scrollTo({ top: 0, behavior: 'smooth' }); setPageIndex(0); setEdit(false);
    if (summary._id === 'welcome') { setDream(summary); setScreen('dream'); return; }
    try {
      const response = await fetch(`${api}/dreams/${summary._id}`);
      if (!response.ok) throw new Error();
      setDream(await response.json()); setScreen('dream');
    } catch {
      setDream(summary); setScreen('dream');
    }
  };
  ...
  const createDream = async () => {
    ...
    try {
      ...
      created.author = { _id: user.id, username: user.username }; setDreams(list => [created, ...list]); setDream(created);
    } catch {
      setDream({ ...newDream, _id: `local-${Date.now()}`, author: { _id: user.id, username: user.username } });
    }
    setEdit(true); setPageIndex(0); setScreen('dream'); window.scrollTo({ top: 0, behavior: 'smooth' });
  };
  ...
  const saveDream = async () => {
    ...
    try {
      ...
      setDream(saved); await loadDreams();
  ...
  const deletePage = () => {
    ...
    setDream({ ...dream, pages }); setPageIndex(Math.min(pageIndex, pages.length - 1));
  };
  ...
  const turnPage = next => {
    ...
    setPageLoading(true); setTimeout(() => { setPageIndex(next); setPageLoading(false); window.scrollTo({ top: 0, behavior: 'smooth' }); }, 800);
  };
  ...
  return <>
    ...
    {pageLoading && <div className="page-loader" aria-label="Turning the page"><span className="loader-moon" /><p>turning the page</p></div>}
    <header><button className="brand" onClick={goHome} aria-label="PaperMoon home"><span className="moon" /><span>paper</span><b>moon</b></button><nav>{admin && <button className="admin" onClick={openAdmin}>Admin</button>}<button className="new" onClick={createDream}>Write a dream</button>{user ? <button className="account" onClick={() => { localStorage.clear(); setUser(null); setToken(null); }}>@{user.username} · sign out</button> : <button className="account" onClick={() => setAuthOpen(true)}>Sign in</button>}</nav></header>
    {screen === 'home' ? <Home dreams={dreams} loading={dreamsLoading} onOpen={openDream} onCreate={createDream} /> : screen === 'admin' ? <Admin dreams={dreams} onBack={goHome} onDelete={deleteDream} /> : <main className="reader"><button className="back" onClick={goHome}>← All dreams</button><div className="story-head"><div><p className="eyebrow">A PAPERMOON DREAM</p>{edit ? <input value={dream.title} onChange={event => setDream({ ...dream, title: event.target.value })} /> : <h1>{dream.title}</h1>}{edit ? <input className="subtitle-input" value={dream.subtitle} placeholder="Add a subtitle" onChange={event => setDream({ ...dream, subtitle: event.target.value })} /> : <p className="subtitle">{dream.subtitle}</p>}<p className="byline">Words and photographs by <b>{dream.author?.username}</b></p></div>{own && <button className="edit-btn" onClick={() => { if (edit) saveDream(); setEdit(!edit); }}>{edit ? 'Save dream' : 'Edit dream'}</button>}</div>
      <div className="cover" style={{ backgroundImage: `url(${cover})` }}>{edit && pageIndex === 0 && <label><input type="file" accept="image/*" onChange={event => upload(event.target.files[0], value => setDream({ ...dream, coverImage: value }))} /><span>{dream.coverImage ? 'Change cover photo' : 'Add a cover photo'}</span></label>}<div><p>PaperMoon dream</p><strong>{dream.title}</strong></div></div>
      <article className="paper"><Text text={page.topText} edit={edit} onChange={value => updatePage('topText', value)} cls="d1" /><div className="d2"><Text text={page.leftText} edit={edit} onChange={value => updatePage('leftText', value)} cls="d21" /><Image src={page.rightImage} edit={edit} onFile={file => upload(file, value => updatePage('rightImage', value))} cls="d22" /></div><div className="d3"><Image src={page.leftImage} edit={edit} onFile={file => upload(file, value => updatePage('leftImage', value))} cls="d31" /><Text text={page.rightText} edit={edit} onChange={value => updatePage('rightText', value)} cls="d32" /></div><Text text={page.bottomText} edit={edit} onChange={value => updatePage('bottomText', value)} cls="d4" /></article>
...
function Home({ dreams, loading, onOpen, onCreate }) { return <main className="home"><section className="hero"><div className="home-moon" aria-hidden="true" /><span className="hero-star star-one" aria-hidden="true">✦</span><span className="hero-star star-two" aria-hidden="true">✧</span><span className="hero-star star-three" aria-hidden="true">✦</span><p className="eyebrow">DREAMS, PAGE BY PAGE</p><h1>A quieter place for <em>your</em> dreams.</h1><p>PaperMoon lets you create visual dreams with words and photographs, arranged like a small printed journal.</p><button className="hero-button" onClick={onCreate}>Start dreaming <span>→</span></button></section><section className="library"><div className="library-head"><p className="eyebrow">EXPLORE THE DREAMS</p><p>{dreams.length} dreams waiting to be opened</p></div><div className="card-grid">{loading && dreams.length === 0 ? <div className="dreams-loader"><span className="loader-moon" /><p>Gathering dreams</p><small>Waking up the archive…</small></div> : dreams.length ? dreams.map((item, index) => <button className="story-card" style={{ '--delay': `${index * 80}ms` }} key={item._id} onClick={() => onOpen(item)}><img src={item.coverImage || defaultCover} alt="" /><span className="card-shade" /><div><small>BY {item.author?.username}</small><strong>{item.title}</strong><em>{item.subtitle || 'A dream in the making'}</em></div></button>) : <div className="empty-dreams"><strong>The wall is quiet.</strong><span>Be the first to begin a dream.</span></div>}</div></section></main>; }
function Admin({ dreams, onBack, onDelete }) { return <main className="admin-page"><button className="back" onClick={onBack}>← Back to dreams</button><p className="eyebrow">ADMINISTRATION</p><h1>Manage dreams</h1><p className="admin-intro">{dreams.length} published dreams</p><div className="dream-list">{dreams.map(item => <div className="dream-row" key={item._id}><div><strong>{item.title || 'Untitled dream'}</strong><small>by {item.author?.username || 'unknown'}</small></div><button className="delete" onClick={() => onDelete(item._id)} aria-label={`Delete ${item.title}`}>⌫ <span>Delete</span></button></div>)}</div></main>; }
function Auth({ form, setForm, mode, setMode, error, close, submit }) { return <div className="modal"><form onSubmit={submit}><button type="button" className="close" onClick={close}>×</button><div className="auth-moon" /><p className="eyebrow">WELCOME TO PAPERMOON</p><h2>{mode === 'login' ? 'Your dreams are waiting.' : 'Make room for your dreams.'}</h2><p className="auth-copy">{mode === 'login' ? 'Sign in to write, edit, and return to your pages.' : 'A simple account is all you need to begin.'}</p><input placeholder="Choose a username" required value={form.username} onChange={event => setForm({ ...form, username: event.target.value })} /><input placeholder="Create a password" type="password" required value={form.password} onChange={event => setForm({ ...form, password: event.target.value })} />{error && <small className="error">{error}</small>}<button className="submit">{mode === 'login' ? 'Sign in' : 'Create account'} <span>→</span></button><button type="button" className="switch" onClick={() => setAuthMode(mode === 'login' ? 'register' : 'login')}>{mode === 'login' ? 'New here? Create an account' : 'Already have an account? Sign in'}</button></form></div>; }
